       ...
       <section class="content-header">
                   ...
                   <div class="row">
		            ...
		            <div class="row">
	                    ...
	                    <div class="col-md-6" style="">
	                    	<div class="form-group">
	                      		<label class="col-md-12" style="padding-top: 7px; margin-bottom: 0"><a href="javascript:void(0);" onclick="addAdvertiser();"><spring:message code="business.opportunity.add.advertiser" /></a></label>
	                     	</div>
	                    </div>
	                </div>
		...
		<script>    
			$(document).ready(function() {
				...
				$("#deliver_date").daterangepicker({opens:"right",cancelClass:"btn-info",format:'YYYY/MM/DD',locale :{
				...
				$("#progress").ionRangeSlider({
					...
					max:100,
					force_edges: false,
					<c:if test="${ action eq 'update' && businessOpportunity.progress==10 }">
					values:[0,10,30],
					</c:if>
					<c:if test="${ action eq 'create' || businessOpportunity.progress !=10 }">
					values:[0,10,30,50,70,90,100],
					</c:if>
					<c:if test="${action eq 'create' }">from_fixed: true,</c:if>
					<c:if test="${action eq 'update' }">
						<c:if test="${businessOpportunity.progress == 100}">
							from_fixed: true,
						</c:if>
						<c:if test="${businessOpportunity.progress != 100}">
							from_fixed: false,
					...
					prettify: function (num) { 
						if(num==0) return '<spring:message code="business.opportunity.status.lost" />'+": "+num+"%";
						if(num==10) return '<spring:message code="business.opportunity.status.inprogress" />'+": "+num+"%";
						if(num==30||num==50||num==70||num==90) return '<spring:message code="business.opportunity.status.won" />'+": "+num+"%";
						if(num==100) return '<spring:message code="business.opportunity.status.signed" />'+": "+num+"%";
				    }
				});
				...
				<c:if test="${action eq 'update' }">
					...
					if( '${businessOpportunity.status}' >= 3 ){
						disabled_from(true);
					}
				</c:if>
			});
