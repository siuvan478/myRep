						...
						<table class="table table-striped dataTable table-hover" style="margin-bottom:0px;">
							<thead>
								<tr>
									...
									<th><spring:message code="report.CustPaymentInfo.custRemark" /></th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${pages.content}" var="custPaymentInfo" varStatus="index">
									<tr class="${index.count%2==0?'odd':'even'}">
										...
										<td>
											...
											%>
												${custPaymentInfo.currencyNameZH}
											<% }else { %>
												${custPaymentInfo.currencyNameEN}
											<% } %>
											
										</td>
										<td class="text-right">
											<fmt:formatNumber value="${custPaymentInfo.convertToRMB}" type="currency" pattern="#0.00" />
										</td>
										<td class="text-right">
											<fmt:formatNumber value="${custPaymentInfo.topupAmount}" type="currency" pattern="#0.00" />
										</td>
										<td class="text-right">
											<fmt:formatNumber value="${custPaymentInfo.discountAmount}" type="currency" pattern="#0.00" />
										</td>
										<td class="text-right">
