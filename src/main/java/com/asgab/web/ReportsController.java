...
public class ReportsController {
	...
	public String downloadCustBaseInfo(ServletRequest request, HttpServletResponse response) throws IOException {
		...
		response.setHeader("Content-Disposition", "attachment;filename=\"Costomer Basic Information.xlsx\"");
		...
		if (lists != null && lists.size() > 0 && workbook != null) {
			...
			for (int i = 0; i < lists.size(); i++) {
				...
				if (row != null) {
					...
					row.createCell(1).setCellValue(info.getCustName());
					row.createCell(2).setCellValue(info.getAdvertiser());
					row.createCell(3).setCellValue(info.getPayerName());
					row.createCell(4).setCellValue(info.getCustPort());

					XSSFCell cell5 = row.createCell(5);
					cell5.setCellValue(info.getTopupDate());
					cell5.setCellStyle(dateStyle);

					row.createCell(6).setCellValue(
							request.getLocale().getLanguage().equalsIgnoreCase("zh") ? info.getCurrencyNameZH() : info.getCurrencyNameEN());
					row.createCell(7).setCellValue(CommonUtil.formatNumber(info.getConvertToRMB(), 2));
					row.createCell(8).setCellValue(CommonUtil.formatNumber(info.getTopupAmount(), 2));
					row.createCell(9).setCellValue(CommonUtil.formatNumber(info.getDiscountAmount(), 2));
					row.createCell(10).setCellValue(CommonUtil.formatNumber(info.getAcctCreateFeeAmount(), 2));
					row.createCell(11).setCellValue(CommonUtil.formatNumber(info.getAnnualFeeAmount(), 2));
					row.createCell(12).setCellValue(CommonUtil.formatNumber(info.getGuaranteeFeeAmount(), 2));
					row.createCell(13).setCellValue(CommonUtil.formatNumber(info.getMgtFeeAmount(), 2));
					row.createCell(14).setCellValue(info.getCustRemark());
				}
			}
		}
